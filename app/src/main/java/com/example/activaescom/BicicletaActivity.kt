...
class BicicletaActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private fun cargarDatosConfiguracion() {
        ...
        tvConfigRutinaTitulo.text = intent.getStringExtra("NOMBRE_RUTINA") ?: "Bicicleta Individual"
        tvConfigMeta.text = "Meta: ${intent.getStringExtra("META_DISTANCIA") ?: "20.0 km"}"
        tvConfigSueno.text = "💤 : ${intent.getIntExtra("HORAS_SUENO", 7)} hrs"
        tvConfigLugar.text = "📍 : ${intent.getStringExtra("LUGAR_ENTRENAMIENTO") ?: "Exterior"}"
    ...
    private fun processNewLocation(location: Location) {
        ...
        googleMap?.animateCamera(CameraUpdateFactory.newLatLngZoom(newPoint, 16f))
    }
    ...
    private fun updateStatsUI() {
        ...
        if (elapsedSeconds > 0 && totalDistanceMeters > 0) {
            val avgSpeedKmh = (totalDistanceMeters / elapsedSeconds.toDouble()) * 3.6
            tvVelocidad.text = String.format("%.1f", avgSpeedKmh)
        }
    }
